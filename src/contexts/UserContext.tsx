...
export const UserProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [followDataFetched, setFollowDataFetched] = useState<Set<number>>(
    new Set()
  );
  const [followerDataFetched, setFollowerDataFetched] = useState<Set<number>>(
    new Set()
  );
  ...
  const isFollowing = (
    ...
    const result = followRelations.some(
      (relation) =>
        relation.follower_user_id === followerUserId &&
        relation.followed_user_id === followedUserId
    );
  ...
  const isFollowedBy = (userId: number, followerId: number): boolean => {
    return followRelations.some(
      (relation) =>
        relation.follower_user_id === followerId &&
        relation.followed_user_id === userId
    );
  };

  const getFollowRelation = (
    followerUserId: number,
    followedUserId: number
  ): FollowRelation | null => {
    return (
      followRelations.find(
        (relation) =>
          relation.follower_user_id === followerUserId &&
          relation.followed_user_id === followedUserId
      ) || null
  ...
  const getFollowers = (userId: number): FollowRelation[] => {
    ...
    return followRelations.filter(
      (relation) => relation.followed_user_id === userId
    );
  };
  ...
  const getFollowing = (userId: number): FollowRelation[] => {
    ...
    return followRelations.filter(
      (relation) => relation.follower_user_id === userId
    );
  };
  ...
  const fetchUserInternal = async (userId: number): Promise<UserProfile> => {
    if (isNaN(userId)) {
      ...
      setUsers((prev) =>
        prev.some((user) => user.id === 0) ? prev : [...prev, emptyUser]
      );
      return emptyUser;
    }
  ...
  const fetchBulkUsersFromRelations = async (
    ...
    try {
      ...
      setUsers((prev) =>
        prev.map((user) => {
          const realUser = bulkUsers.find(
            (bulkUser) => bulkUser.id === user.id
          );
  ...
  const fetchUserFollowsInternal = async (userId: number): Promise<void> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to fetch user follows";
  ...
  const fetchUserFollowersInternal = async (userId: number): Promise<void> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to fetch user followers";
      setError(errorMessage);
  ...
  const followUser = async (
    ...
    console.log(
      `Starting follow process: ${followerUserId} -> ${followedUserId}`
    );
    ...
    try {
      ...
      // Make the API call
      const realFollowRelation = await usersApi.followUser(
        followerUserId,
        followedUserId
      );
      ...
      setFollowRelations((prev) => {
        const updatedRelations = prev.map((relation) =>
          relation.id === optimisticFollowRelation.id
            ? realFollowRelation
            : relation
        );
        console.log(
          "Replaced optimistic with real relation:",
          updatedRelations
        );
      ...
      setFollowRelations((prev) => {
        const filteredRelations = prev.filter(
          (relation) => relation.id !== optimisticFollowRelation.id
        );
        console.log(
          "Removed optimistic relation due to error:",
          filteredRelations
        );
  ...
  const unfollowUser = async (
    ...
    console.log(
      `Starting unfollow process: ${followerUserId} -> ${followedUserId}`
    );
    ...
    setFollowRelations((prev) => {
      const filteredRelations = prev.filter(
        (relation) => relation.id !== followRelation.id
      );
      console.log(
        "Updated follow relations after unfollow:",
        filteredRelations
      );
      return filteredRelations;
    });
    ...
    try {
      ...
      setFollowRelations((prev) => {
        ...
        console.log(
          "Restored follow relation due to error:",
          restoredRelations
        );
