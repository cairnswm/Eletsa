...
export const organizersApi = {
  ...
  async createOrganizer(
    organizerData: CreateOrganizerRequest
  ): Promise<Organizer> {
    const response = await fetch(`${ORGANIZER_API}/api.php/organizer`, {
      method: "POST",
  ...
  async updateOrganizer(
    id: number,
    organizerData: UpdateOrganizerRequest
  ): Promise<Organizer> {
    const response = await fetch(`${ORGANIZER_API}/api.php/organizer/${id}`, {
      method: "PUT",
      headers: createHeaders(true),
  ...
  async fetchOrganizerEvents(userId: number): Promise<Event[]> {
    const response = await fetch(
      `${ORGANIZER_API}/api.php/organizer/${userId}/events`,
      {
        method: "GET",
        headers: createHeaders(),
      }
    ...
    return events.map((event) => ({
      ...
      location_latitude: event.location_latitude
        ? Number(event.location_latitude)
        : 0,
      location_longitude: event.location_longitude
        ? Number(event.location_longitude)
        : 0,
      popularity_score: event.popularity_score
        ? Number(event.popularity_score)
        : 0,
      max_attendees: event.max_attendees ? Number(event.max_attendees) : 0,
    }));
  },

  async fetchOrganizerPayoutRequests(
    organizerId: number
  ): Promise<PayoutRequest[]> {
    const response = await fetch(
      `${ORGANIZER_API}/api.php/organizer/${organizerId}/payout_requests`,
      {
        method: "GET",
        headers: createHeaders(),
      }
  ...
  async fetchOrganizerPayouts(organizerId: number): Promise<Payout[]> {
    const response = await fetch(
      `${ORGANIZER_API}/api.php/organizer/${organizerId}/payouts`,
      {
        method: "GET",
        headers: createHeaders(),
      }
  ...
  async updatePayout(
    id: number,
    payoutData: UpdatePayoutRequest
  ): Promise<Payout> {
    const response = await fetch(`${ORGANIZER_API}/api.php/payouts/${id}`, {
      method: "PUT",
      headers: createHeaders(true),
  ...
  async fetchTransaction(id: number): Promise<Transaction> {
    const response = await fetch(
      `${ORGANIZER_API}/api.php/transactions/${id}`,
      {
        method: "GET",
        headers: createHeaders(),
      }
