...
export const eventsApi = {
  ...
  async fetchEventTicketTypes(eventId: number) {
    const response = await fetch(
      `${EVENTS_API}/api.php/event/${eventId}/ticket_types`,
      {
        method: "GET",
        headers: createHeaders(),
      }
    ...
    return ticketTypes.map((ticketType) => ({
      ...
      quantity_sold: ticketType.quantity_sold
        ? Number(ticketType.quantity_sold)
        : 0,
    }));
  },
  ...
  async fetchEventComments(eventId: number) {
    const response = await fetch(
      `${EVENTS_API}/api.php/event/${eventId}/comments`,
      {
        method: "GET",
        headers: createHeaders(),
      }
  ...
  async updateTicketType(
    ...
    const response = await fetch(
      `${EVENTS_API}/api.php/ticket_type/${ticketTypeId}`,
      {
        method: "PUT",
        headers: createHeaders(true),
        body: JSON.stringify(ticketData),
